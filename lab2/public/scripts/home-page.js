...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function closeCategoriesPopup() {
    ...
    categoriesPopup.addEventListener("transitionend", function handler(e) {
      if (
        ...
        !categoriesPopup.classList.contains("active")
      ) {
  ...
  fetch("/home/getCategories")
    ...
    .then((categories) => {
      ...
      [...categoriesPopup.querySelectorAll(".kategorije")].forEach((el) =>
        el.remove()
      );
...
function prikaziProizvodeSaLoadMore(products, reset) {
  ...
  const toShow = products.slice(
    currentlyShown,
    currentlyShown + PRODUCTS_PER_LOAD
  );
  toShow.forEach((p) => {
    productsDiv.innerHTML += `
      <div class="product">
  ...
  loadMoreBtn.style.display =
    currentlyShown < products.length ? "block" : "none";
